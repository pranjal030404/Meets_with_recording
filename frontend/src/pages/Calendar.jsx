        ...
        <div className="bg-dark-200 rounded-2xl shadow-xl p-6 border border-dark-400">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={prevMonth}
                className="p-2 hover:bg-dark-400 rounded-lg transition text-gray-300 hover:text-white"
              >
                <ChevronLeft className="w-5 h-5" />
              </button>
              <button
                onClick={() => setCurrentDate(new Date())}
                className="px-4 py-2 text-sm font-medium text-gray-300 hover:bg-dark-400 hover:text-white rounded-lg transition"
              >
                Today
              </button>
              <button
            ...
            {days.map((day, index) => {
              ...
              return (
                <div
                  ...
                  className={`min-h-24 p-2 border rounded-xl transition-all duration-200 ${
                    day
                      ? isPast
                        ? 'bg-dark-400/30 cursor-not-allowed border-dark-400'
                        : 'bg-dark-300 hover:bg-dark-400 cursor-pointer border-dark-400 hover:border-primary-500'
                      : 'bg-dark-400/20 border-dark-400'
                  } ${isToday ? 'border-primary-500 border-2 ring-2 ring-primary-500/20' : ''}`}
                >
                  {day && (
                    <>
                      <div className={`text-sm font-medium mb-1 ${
                        isToday ? 'text-primary-400' : isPast ? 'text-gray-600' : 'text-white'
                      }`}>
                        {day.getDate()}
                      </div>
                      <div className="space-y-1">
                        {dayMeetings.slice(0, 2).map(meeting => (
                          <div
                            ...
                            className="text-xs bg-primary-600/20 text-primary-300 px-2 py-1 rounded-lg truncate hover:bg-primary-600/40 transition-colors"
                            title={meeting.title}
                          >
                            ...
                            <div className="text-primary-400 text-[10px]">
      ...
      {showScheduleModal && (
        <div className="modal-backdrop">
          <div className="modal-content max-w-2xl max-h-[90vh] overflow-y-auto">
            <h2 className="text-2xl font-bold mb-4 text-white">Schedule Meeting</h2>
